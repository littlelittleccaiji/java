...
public class eightQueenBookResult {
    public static void main(String[] args) {
        cal8queens(0);
    }
    static int[] result = new int[8];//全局或成员变量,下标表示行,值表示queen存储在哪一列

    public static void cal8queens(int row) {//调用方式：cal8queens(0);
        if (row == 8) {//8个棋子都放置好了，打印结果
            printQueens(result);
            return;//8行棋子都放好了，已经没法再往下递归了，所以就return
        }
        for (int column = 0; column < 8; ++column) {//每一行都有8中放法
            if (isOk(row, column)) {//有些放法不满足要求
                result[row] = column;//第row行的棋子放到了column列
                cal8queens(row + 1);//考察下一行
            }
        }
    }
    ...
    private static boolean isOk(int row, int column) {//判断row行column列放置是否合适
        ...
        for (int i = row - 1; i >= 0; --i) { // 逐行往上考察每一行
            ...
            if (rightup < 8) { // 考察右上对角线：第i行rightup列有棋子吗？
